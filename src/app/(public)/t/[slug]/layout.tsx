...
export default function TripLayout({ children, params }: LayoutProps): React.JSX.Element {
  ...
  const isImm       = pathname === `/t/${slug}/imm`;
  const isRating    = pathname === `/t/${slug}/rating`;
  ...
  return (
    <>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full z-50 bg-white/95 backdrop-blur-xl border-t border-slate-100 shadow-[0_-1px_0_0_rgba(0,0,0,0.06),0_-8px_24px_rgba(0,0,0,0.06)]">
        <div className={`grid ${isFollowing ? "grid-cols-3" : "grid-cols-3"} items-end pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2 px-2`}>

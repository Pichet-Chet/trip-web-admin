...
export default function GoogleLinkCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (errorParam) {
      setError("ยกเลิกการเชื่อมต่อ Google");
    ...
    const savedState = sessionStorage.getItem("google_link_state");
    ...
    const redirectUri = `${window.location.origin}/google-link-callback`;
    api
      .post("/admin/me/google/link", { code, redirectUri })
      .then(() => router.replace("/dashboard/settings?linked=google"))
      .catch((err) => {
        setError(err instanceof ApiError ? err.message : "เชื่อมต่อ Google ไม่สำเร็จ");
      ...
      <div className="text-center max-w-sm px-6">
        {error ? (
          ...
          <>
            ...
            <p className="text-(--on-surface) font-semibold mb-1">กำลังเชื่อมต่อ Google</p>

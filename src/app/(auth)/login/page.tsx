...
function LoginPageContent(): React.ReactNode {
  ...
  async function handleGoogleLogin() {
    try {
      ...
      const res = await api.get<{ url: string; state: string }>(
        `/client/auth/google/authorize-url?redirectUri=${encodeURIComponent(redirectUri)}`
      );
      sessionStorage.setItem("google_oauth_state", res.state);
      window.location.href = res.url;
    } catch (err) {
      setApiError(err instanceof ApiError ? err.message : "ไม่สามารถเชื่อมต่อ Google ได้");
    }
  }
  ...
  async function handleLineLogin() {
    try {
      ...
      const res = await api.get<{ url: string; state: string }>(
        `/client/auth/line/authorize-url?redirectUri=${encodeURIComponent(redirectUri)}`
      );
      sessionStorage.setItem("line_oauth_state", res.state);
      window.location.href = res.url;
    } catch (err) {
      setApiError(err instanceof ApiError ? err.message : "ไม่สามารถเชื่อมต่อ LINE ได้");
    }
  }
      ...
      <section className="flex-1 flex items-center justify-center p-6 sm:p-12 lg:p-20 bg-(--surface)">
        <div className="w-full max-w-md">
          ...
          <div className="mb-8 flex flex-col gap-3">
            {/* Google Login */}
            <button type="button" onClick={handleGoogleLogin} className="w-full flex items-center justify-center gap-3 bg-white border border-(--outline-variant)/30 hover:bg-(--surface-container-low) transition-all text-(--on-surface) py-3.5 px-6 rounded-xl font-semibold shadow-sm active:scale-[0.98] duration-200">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 bg-[#06C755] hover:bg-[#05b34c] transition-all text-white py-3.5 px-6 rounded-xl font-semibold shadow-sm active:scale-[0.98] duration-200"
            >

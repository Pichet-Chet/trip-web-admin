...
# [Platform Name] — Trip Communication Platform
...
[Platform] คือแพลตฟอร์ม SaaS สำหรับบริษัททัวร์ ไกด์อิสระ และผู้ใช้ส่วนตัว ที่ช่วยสร้าง จัดการ และสื่อสาร itinerary กับลูกทริปแบบ real-time โดยเน้นแก้ปัญหาหลักคือ **"แก้ plan แล้วลูกทริปไม่รู้"** ผ่านระบบแจ้งเตือนอัตโนมัติ ปุ่มรับทราบ และ read receipt
...
- สร้าง unique URL: `[your-domain.com]/t/{slug}`
...
- แสดง:
  ...
  - "Powered by [Platform]" badge (free tier)
...
- เลือกช่องทางรับแจ้งเตือน:
  - **LINE OA (Messaging API)** — User เพิ่มเพื่อน LINE OA ของระบบ → link trip (เหมาะคนไทย)
...
- ระบบสร้าง change summary อัตโนมัติ:
  ...
  👉 ดูรายละเอียด: [your-domain.com]/t/chiangmai3d
  ```
...
- ระบบส่ง notification อัตโนมัติผ่านทุกช่องทาง (LINE Messaging API + Web Push) ก่อนวันเดินทางแต่ละวัน (เช่น 20:00 คืนก่อน)
  ...
  👉 ดูรายละเอียด: [your-domain.com]/t/tokyo8d
  ```
...
- Free tier: แสดง "Powered by [Platform]" + link ถาวร
...
- URL: `[your-domain.com]/p/{custom-slug}`
...
- **Flow:**
  1. ลูกทริปกด "ติดตาม" → User เพิ่มเพื่อน LINE OA ของระบบ → link trip
...
*Document End — [Platform] System Requirements v2.0*

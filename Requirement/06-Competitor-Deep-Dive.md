...
> **[Platform] ไม่มีคู่แข่งตรงในตลาดไทย** — ไม่มี software รายใดที่แก้ปัญหา "แก้ plan แล้วลูกทริปไม่รู้" โดยตรง
>
> **ในต่างประเทศมีคู่แข่งใกล้เคียง** (AXUS, Vamoos) แต่แพงกว่า 10-100 เท่า ต้อง download app และไม่มี LINE integration
>
> **คู่แข่งตัวจริงของ [Platform] ���ือ "ความเคยชินที่จะใช้ LINE Group + PDF"**

---

## 1. [Platform] Business Overview (สำหร���บเปรียบเทียบ)

[Platform] มี 2 ส่วนหลักที่เป็นอิสระต่อกัน:
...
**[Platform] อยู่ฝั่งซ้าย (Itinerary Communication) ซึ่งว่างเปล่า — ไม่มีใครอยู่เลย**
...
| **เทียบ [Platform]** | TourSys = "จัดการธุรกิจทัวร์" vs [Platform] = "สื่อสารกับลูกทริป" |
...
| **เทียบ [Platform]** | Salesone = "ขายทัวร์ออนไลน์" vs [Platform] = "service ลูกทริปหลังขาย" |
...
| **เทียบ [Platform]** | TourProX = "กระจายทัวร์ให้ agent" — ไม่เกี่ยวกับ [Platform] |
...
#### เปรียบเทียบ: เว็บขายทัวร์ vs [Platform] Posts

| เรื่อง | เว็บขายทัว���์สำเร็จรูป | [Platform] Posts |
|---|---|---|
| **ราคา** | หลักพัน - หลักหมื่น (ค่าทำ + รายเดือน) | **ฟรี** (โพสบน platform) |
| **เป็นเจ้าของ** | บริษัทมีเว็บของตัวเอง | อยู่บน platform [Platform] |
...
| **เทียบ [Platform]** | Kaitour = "ตลาดนัดทัวร์สำหรับบริษัทใหญ่" vs [Platform] Posts = "พื้นที่ให้ทุกคนโชว์ทัวร์" |
...
#### เปรียบเทียบ: Aggregator vs [Platform] Posts

| เรื่อง | Kaitour / Aggregator | [Platform] Posts |
|---|---|---|
...
| **จุดด้อย** | B2C only, ไม่มี B2B features, ไม่มี notification, ไม่มี itinerary sharing แบบ [Platform] |
| **เทียบ [Platform]** | TripNiceDay = "ค้นหาที่เที่ยว + diary" vs [Platform] = "สร้าง plan แชร์เพื่อน + service ลูกทัวร์" |
...
> **คู่แข่งตัวจริงของ [Platform] = "ความเค��ชิน" ที่จะใช้ LINE Group + PDF ไม่ใช่ software อื่น**
...
#### AXUS Travel App (USA) — คู่แข่งใกล้ [Platform] ที่สุดในโลก
...
| **เทียบ [Platform]** | AXUS มี change notification เหมือน [Platform] แต่ต้อง download app + แพงกว่า 25-100 เท่า + ไม่มี LINE |
...
| **เทียบ [Platform]** | Vamoos มี feature ใกล้เคียง [Platform] มาก แต่แพงกว่า 10-50 เท่า + ต��องลง app |
...
| **เทียบ [Platform]** | mTrip = "enterprise solution สำหรับบริษัทใหญ่" vs [Platform] = "เครื่องมือง่ายๆ สำหรับ SME" |
...
| **เทียบ [Platform]** | SquadTrip = "เก็บเงินกรุ๊ปทัวร์" vs [Platform] = "สื่อสาร itinerary" |
| **ระดับคุกคาม** | **ต่ำ** — คนละ focus |
...
| Feature | LINE Group | PDF | AXUS | Vamoos | mTrip | **[Platform]** |
|---|---|---|---|---|---|---|
...
| Feature | Weon/OKWebTour | Kaitour | Klook/KKday | Facebook Group | **[Platform] Posts** |
|---|---|---|---|---|---|
...
## 5. [Platform] Unique Advantages (สิ่งที่ไม่มีใครมี)
...
| Moat | คำอธิบาย |
|---|---|
| **Network Effect** | ลูกทริปเห็น "Powered by [Platform]" → viral loop → คู่แข่งต้องสร้าง user base จาก 0 |
| **Posts Platform Effect** | ยิ่งมีบริษัทโพสมาก → ลูกค้ามาหามาก → บริษัทอื���นอยากโพสด้วย |
| **First Mover** | ไม่มีใครทำ Trip Communication ในไทย → [Platform] เป็นรายแรก |
| **Switching Cost** | เมื่อบริษัทมี portfolio + followers บน [Platform] แล้ว ไม่อยากย้าย |
...
| **คนไทยท�� clone [Platform]** | กลาง-สูง — ถ้าเราสำเร็จ | First-mover + viral loop + community lock-in |
| **AXUS/Vamoos เข้าตลาดไทย** | ต่ำ — ราคาส��ง + ไม่มี LINE | เราถูกกว่า 25-100 เท่า + มี LINE integration |
| **บริษัททัวร์ใช้ LINE ต่อเพรา��� "ก็ใช้ได้อยู่"** | **สูง** — inertia แรงมาก | ต้องทำให��� [Platform] ดีกว่า LINE อย��างเห็นได้ชัด |
...
| **ราคา [Platform] ถูกมาก** | AXUS $35+/เดือน vs [Platform] ฿49/ทริป (~$1.4) |
| **Immigration View เป���นจุดขาย** | ทั้ง 2 ประเทศเดินทางเยอะ ใช้ ตม. บ่อย |
| **i18n ภาษาญี่ปุ่นมีแล้ว** | [Platform] มี 3 ภาษา (th/en/ja) |
...
### [Platform] ในตลาดไทย: ไม่มีคู่แ��่งตรง

- **Back-office tools** (TourSys, Salesone) → เน้น booking + CRM ไม่ใช่ communication
- **เว็บขายทัวร์** (Weon, OKWebTour) → ทำเว็บให้แต่ละเจ้า [Platform] เป็น platform กลาง
- **Aggregator** (Kaitour, eTravelWay) → รวมทัวร์ขาย [Platform] เป็นพื้นที่โชว์ + service tool
- **B2C Trip Planner** (TripNiceDay) → ค้นหาที่เที่ยว [Platform] เน้นสร้าง plan + แชร์

### [Platform] ในตลาดโลก: ถูกกว่า + ง่ายกว่า
...
*Document End — [Platform] Competitor Deep Dive v2.0*
